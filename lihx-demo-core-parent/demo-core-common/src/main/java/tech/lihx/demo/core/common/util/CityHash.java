...
public class CityHash {
	...
	private static long toLongLE( byte[] b, int i ) {
		return (((long) b[i + 7] << 56)
				+ ((long) (b[i + 6] & 255) << 48) + ((long) (b[i + 5] & 255) << 40) + ((long) (b[i + 4] & 255) << 32)
	...
	public static long cityHash64( byte[] s, int pos, int len ) {
		...
		} else if ( len <= 64 ) { return hashLen33to64(s, pos, len); }
	...
	public static long[] cityHash128WithSeed( byte[] s, int pos, int len, long seed0, long seed1 ) {
		...
		if ( len < 128 ) { return cityMurmur(s, pos, len, seed0, seed1); }
	...
	public static void main( String[] args ) {
		byte[] b = "我我我我我我我我我我我我".getBytes();
		System.out.println(cityHash64(b, 0, b.length));
	}
}
